...
test.describe('Страница оформления заказа неавторизованным пользователем', () => {
  let buns;
  let sauces;
  let main;
  ...
  test('Заказ не должен оформляться', async ({ page }) => {
    const [bunItem] = buns;
    const [sauceItem] = sauces;
    const [mainItem] = main;
    await page.locator(`a[href="/ingredients/${bunItem._id}"] + button`).click();
    ...
    try {
      orderRequest = await page.waitForRequest(request => 
    ...
    await expect(orderRequest).toBeNull();
  });
});
